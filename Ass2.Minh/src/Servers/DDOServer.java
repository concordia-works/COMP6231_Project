...
public class DDOServer {
    public static void main(String args[]) {
        try {
            ...
            CenterServer servant = new CenterServer(Config.Server_ID.DDO);
            servant.setORB(orb);
            ...
            NameComponent path[] = namingContextRef.to_name(Config.Server_ID.DDO.name());
            namingContextRef.rebind(path, dcmsServer);

            // Run the server
            dcmsServer.startUDPServer();
            System.out.println("Server " + Config.Server_ID.DDO.name() + " is running ...");
            orb.run();
        } catch (Exception e) {
